...
import novoda.widget.{ArticlePageAdapter, ArticlePager, ColumnLayout}
...
class SimpleTextColumn2 extends Activity {
  override def onCreate(bundle: Bundle) {
    super.onCreate(bundle);
    val layout = new ArticlePager(this)

    val context = this
    layout.setAdapter(new PagerAdapter() {
      def getCount = 3;

      def isViewFromObject(view: View, `object`: AnyRef) = view == `object`
      ...
      override def instantiateItem(container: ViewGroup, position: Int) = {
        ...
        val tv3 = new TextView(context)
        val tv4 = new TextView(context)
        ...
        val lp5 = new ColumnLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT)
...
class SimpleTextColumn extends Activity {
  override def onCreate(bundle: Bundle) {
    ...
    val layout2 = new ArticlePager(this)
